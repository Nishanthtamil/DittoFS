...
class TestEncryptedChunk:
    ...
    def test_encrypted_chunk_invalid_data(self):
        ...
        invalid_data = (
            os.urandom(SALT_SIZE + AES_IV_SIZE + HMAC_SIZE) + b"\x00\xff"
        )  # Invalid length
...
class TestChunkEncryption:
    ...
    def test_chunk_encryption_roundtrip(self):
        ...
        # Decrypt the chunk
        decrypted_data = self.chunk_encryption.decrypt_chunk(
    ...
    def test_chunk_integrity_verification(self):
        ...
        assert not self.chunk_encryption.verify_chunk_integrity(
            encrypted_chunk, "wrong_hash"
        )

        # Should fail with corrupted data
        corrupted_chunk = EncryptedChunk(
            encrypted_data=encrypted_chunk.encrypted_data[:-1]
            + b"X",  # Corrupt last byte
...
class TestEncryptionSystemIntegration:
    ...
    def test_create_encryption_system(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            key_manager, chunk_encryption = create_encryption_system(
    ...
    def test_load_encryption_system(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create system first
            key_manager1, chunk_encryption1 = create_encryption_system(
                password, key_store_path
            )
    ...
    def test_encryption_system_end_to_end(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for chunk_data, chunk_hash in test_chunks:
                ...
                # Verify integrity
                assert chunk_encryption.verify_chunk_integrity(
                    encrypted_chunk, chunk_hash
                )
            ...
            for i, (encrypted_chunk, chunk_hash) in enumerate(encrypted_chunks):
                decrypted_data = chunk_encryption2.decrypt_chunk(
                    encrypted_chunk, chunk_hash
                )
