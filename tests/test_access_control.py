...
class TestAccessControlService:
    ...
    async def test_get_role_permissions_with_inheritance(self, access_control_service):
        ...
        effective_permissions = await access_control_service.get_role_permissions("child_role")
    ...
    async def test_remove_user_from_group(self, access_control_service):
        ...
        await access_control_service.create_group("test_group", "Description", {"user1"})
        
        # Remove user from group
        result = await access_control_service.remove_user_from_group("user1", "test_group")
        assert result is True
        
        group = await access_control_service.get_group("test_group")
        assert "user1" not in group.members
        assert "test_group" not in access_control_service.user_groups.get("user1", set())
        
        # Remove user not in group
        result = await access_control_service.remove_user_from_group("user2", "test_group")
        assert result is False
    ...
    async def test_get_user_permissions(self, access_control_service):
        ...
        await access_control_service.create_role("custom_role", "Custom", custom_permissions)
    ...
    async def test_check_permission(self, access_control_service):
        ...
        # Check permission
        has_read = await access_control_service.check_permission("user1", Permission.READ)
        assert has_read is True
        
        has_admin = await access_control_service.check_permission("user1", Permission.ADMIN)
        assert has_admin is True
        
        # Check permission for user without role
        has_admin_user2 = await access_control_service.check_permission("user2", Permission.ADMIN)
    ...
    async def test_persistence(self, access_control_service):
        ...
        # Create test data
        await access_control_service.create_role("test_role", "Test role", {Permission.READ})
        ...
        await access_control_service.add_acl_entry("/test/file", "user1", "user", {Permission.WRITE})
        ...
        try:
            ...
            has_permission = await new_service.check_permission("user1", Permission.READ)
