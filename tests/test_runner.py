...
class TestRunner:
    ...
    def run_unit_tests(self, verbose: bool = False, coverage: bool = True) -> bool:
        ...
        if coverage:
            cmd.extend(
                ["--cov=src/dittofs", "--cov-report=term-missing", "--cov-report=html"]
            )
    ...
    def run_property_tests(self, verbose: bool = False, examples: int = 100) -> bool:
        ...
        cmd.extend(["-m", "property", f"--hypothesis-show-statistics", "tests/"])
    ...
    def run_performance_tests(self, verbose: bool = False) -> bool:
        ...
        cmd.extend(
            ["-m", "benchmark", "--benchmark-only", "--benchmark-sort=mean", "tests/"]
        )
    ...
    def run_parallel_tests(self, workers: int = 4, verbose: bool = False) -> bool:
        ...
        cmd.extend([f"-n{workers}", "-m", "not slow", "tests/"])
    ...
    def lint_code(self) -> bool:
        ...
        flake8_result = self.run_command(
            ["python", "-m", "flake8", "src/", "tests/"], cwd=self.project_root
        )

        # Run mypy
        mypy_result = self.run_command(
            ["python", "-m", "mypy", "src/"], cwd=self.project_root
        )

        return all(
            r.returncode == 0 for r in [black_result, flake8_result, mypy_result]
        )
    ...
    def format_code(self) -> bool:
        ...
        result = self.run_command(
            ["python", "-m", "black", "src/", "tests/"], cwd=self.project_root
        )
    ...
    try:
        if args.command == "unit":
            ...
            success = runner.run_property_tests(
                verbose=args.verbose, examples=args.examples
            )
