...
class TestConflictResolutionWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_auto_resolve_conflict_failure(
        self, conflict_workflow, sample_conflict
    ):
    ...
    async def test_batch_auto_resolve(self, conflict_workflow):
        ...
        conflict_workflow.crdt_store.get_pending_conflicts = Mock(
        ...
        for i in range(5):
            conflict_workflow._add_resolution_entry(
                ...
                (
                    ResolutionAction.KEEP_LOCAL
                    if i % 2 == 0
                    else ResolutionAction.MERGE_AUTOMATIC
                ),
                ResolutionResult.SUCCESS,
                ConflictResolutionStrategy.AUTOMATIC_MERGE,
            )
...
class TestConflictDiff:
    ...
    def test_conflict_diff_creation(self):
        ...
        local_record = FileRecord(
            path="/test/file.txt", hashes=["hash1"], mtime=1000, size=100
        )

        remote_record = FileRecord(
            path="/test/file.txt", hashes=["hash2"], mtime=2000, size=200
        )
