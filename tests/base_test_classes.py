...
from .mocks import (
    ...
    MockSyncManager, NetworkCondition, create_integrated_mock_system
)
from .test_utils import (
    wait_for_condition, assert_eventually, create_file_with_content,
    generate_test_file_content, PerformanceTimer
)
...
class BaseTestCase:
    ...
    def assert_file_chunks_exist(self, chunk_dir: pathlib.Path, hashes: List[str]):
        ...
        for chunk_hash in hashes:
            ...
            assert chunk_path.exists(), f"Chunk {chunk_hash} does not exist at {chunk_path}"
    ...
    def create_test_file(self, temp_dir: pathlib.Path, name: str, content: bytes) -> pathlib.Path:
        """Create a test file with specific content."""
        file_path = temp_dir / name
        return create_file_with_content(file_path, content)
    
    def create_random_file(self, temp_dir: pathlib.Path, size: int, name: Optional[str] = None) -> pathlib.Path:
    ...
    def create_large_file(self, temp_dir: pathlib.Path, chunk_count: int = 3) -> pathlib.Path:
        """Create a large file that spans multiple chunks."""
        size = CHUNK_SIZE * chunk_count + 1000  # Slightly more than exact chunks
        return self.create_random_file(temp_dir, size, f"large_{chunk_count}_chunks.bin")
    ...
    def assert_chunks_are_valid(self, chunk_dir: pathlib.Path, hashes: List[str]):
        ...
        for chunk_hash in hashes:
            ...
            assert len(chunk_content) <= CHUNK_SIZE, (
                f"Chunk {chunk_hash} is too large: {len(chunk_content)} bytes"
            )
    
    def measure_performance(self, operation_name: str = "operation") -> PerformanceTimer:
...
class AsyncTestCase(BaseTestCase):
    ...
    async def wait_for_peer_connection(
        ...
        async def check_connection():
            return peer_id in transport.connections and not transport.connections[peer_id].is_closed
        
        await self.assert_eventually(
            check_connection,
            timeout,
            f"Peer {peer_id} was not connected"
        )
...
class IntegrationTestCase(AsyncTestCase):
    ...
    def assert_peers_eventually_consistent(
        ...
        return self.assert_eventually(
            check_consistency,
            timeout,
            "Peers did not reach consistency"
        )
...
class PerformanceTestCase(AsyncTestCase):
    ...
    def assert_performance_within_bounds(
        ...
        assert metric_name in self.performance_metrics, f"Metric {metric_name} not recorded"
...
class SecurityTestCase(BaseTestCase):
    ...
    def simulate_weak_password_attack(self, password: str = "123456"):
        ...
        self.record_security_event("attack_attempt", {
            "type": "weak_password",
            "password": password
        })
    ...
    def simulate_hash_collision_attack(self, data1: bytes, data2: bytes):
        ...
        self.record_security_event("attack_attempt", {
            "type": "hash_collision",
            "data_sizes": [len(data1), len(data2)]
        })
    ...
    def simulate_timing_attack(self, operation: Callable, *args, **kwargs):
        ...
        self.record_security_event("timing_measurement", {
...
class NetworkTestCase(AsyncTestCase):
    ...
    async def simulate_slow_network(self, latency_ms: int = 1000, duration: float = 1.0):
    ...
    def assert_network_resilience(self, operation: Callable, *args, **kwargs):
        ...
        for conditions in test_conditions:
            ...
            try:
                ...
                pytest.fail(f"Operation failed under network conditions {conditions}: {e}")
