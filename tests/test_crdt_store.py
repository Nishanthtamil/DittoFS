...
def test_branch_operations():
    ...
    branch_id = store.create_branch(
        "feature-branch", main_version_id, "Feature development"
    )
...
def test_retention_policy():
    ...
    for i in range(8):
        ...
        version_id = store.create_file_version(
            path, content, commit_message=f"Version {i}"
        )
...
def test_version_tree_export():
    ...
    feature_version = store.create_file_version(
        path, b"Feature content", "feature", "Feature work"
    )
...
def test_manual_conflict_resolution():
    ...
    manual_resolution = {"resolved_version": record2.to_dict(), "chosen_by_user": True}
...
@pytest.mark.skipif(
    not hasattr(tempfile, "NamedTemporaryFile"), reason="Requires tempfile"
)
def test_conflict_state_machine():
    ...
    store.resolve_conflict(
        conflict.conflict_id, ConflictResolutionStrategy.LAST_WRITER_WINS
    )
