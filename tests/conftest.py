...
from dittofs.chunker import CHUNK_DIR, DEFAULT_AVG_CHUNK_SIZE
...
def test_files() -> Dict[str, TestFileData]:
    ...
    return {
        ...
        "empty_file": TestFileData(
            name="empty.txt", content=b"", size=0, mime_type="text/plain"
        ),
...
def populated_crdt_store(
    clean_crdt_store: CRDTStore, test_files: Dict[str, TestFileData]
) -> CRDTStore:
    ...
    for file_data in test_files.values():
        ...
        fake_hashes = [
            f"hash_{file_data.name}_{i}"
            for i in range(max(1, file_data.size // CHUNK_SIZE + 1))
        ]
...
async def test_peer_factory(temp_dir: pathlib.Path):
    ...
    async def _create_peer(
        peer_id: str, files: Optional[List[TestFileData]] = None
    ) -> TestPeer:
        ...
        with (
            patch("dittofs.chunker.CHUNK_DIR", chunk_dir),
...
def network_simulator():
    ...
    class NetworkSimulator:
        ...
        async def simulate_partition(
            self, peer1_id: str, peer2_id: str, duration: float
        ):
...
class AsyncTestCase(BaseTestCase):
    ...
    async def assert_eventually(
        self, condition_func, timeout: float = 5.0, message: str = "Condition not met"
    ):
