...
from .mocks import (
    ...
    MockSyncManager, MockConnection, NetworkCondition, create_mock_peer_info,
    create_integrated_mock_system
)
from .test_utils import generate_test_file_content, create_file_with_content
...
def test_file_factory(temp_dir: pathlib.Path, test_file_contents: Dict[str, bytes]):
    ...
    def _create_test_file(content_key: str, custom_name: Optional[str] = None) -> pathlib.Path:
...
@pytest.fixture
def populated_crdt_store(clean_crdt_store: MockCRDTStore, test_file_contents: Dict[str, bytes]) -> MockCRDTStore:
    ...
    for name, content in test_file_contents.items():
        ...
        chunk_count = max(1, len(content) // CHUNK_SIZE + (1 if len(content) % CHUNK_SIZE else 0))
...
def network_simulator():
    ...
    class NetworkSimulator:
        ...
        async def simulate_partition(self, duration: float, affected_peers: List[str] = None):
    ...
    async def _create_peer(
        ...
        if files:
            for filename, content in files.items():
                ...
                # Add to mock store
                chunk_count = max(1, len(content) // CHUNK_SIZE + (1 if len(content) % CHUNK_SIZE else 0))
                fake_hashes = [f"hash_{peer_id}_{filename}_{i}" for i in range(chunk_count)]
...
async def hierarchical_peer_network(peer_factory, test_file_contents):
    ...
    hub_peer = await peer_factory("hub", {
        "hub_file.txt": test_file_contents["large_text"]
    })
    ...
    for i in range(4):
        spoke_id = f"spoke_{i}"
        spoke_files = {
            f"spoke_{i}_file.bin": test_file_contents[list(test_file_contents.keys())[i % len(test_file_contents)]]
        }
...
def benchmark_environment():
    ...
    class BenchmarkEnvironment:
        ...
        def record_metric(self, name: str, value: float, unit: str = "seconds"):
            ...
            self.metrics[name] = {"value": value, "unit": unit, "timestamp": time.time()}
...
@pytest.fixture
def integration_test_environment(temp_dir: pathlib.Path, isolated_chunk_dir: pathlib.Path):
