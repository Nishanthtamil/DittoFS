...
class TestDeduplication:
    ...
    def test_partial_deduplication(self):
        ...
        try:
            ...
            assert (
                len(hashes1) > 1
            ), f"File 1 should have multiple chunks, got {len(hashes1)}"
            assert (
                len(hashes2) > 1
            ), f"File 2 should have multiple chunks, got {len(hashes2)}"
            ...
            if dedup_stats["total_chunks"] > dedup_stats["unique_chunks"]:
                ...
                print(
                    f"No deduplication detected - this can happen with content-defined chunking"
                )
                print(
                    f"Total chunks: {dedup_stats['total_chunks']}, Unique: {dedup_stats['unique_chunks']}"
                )
        ...
        try:
            for filename, expected_type in files_and_types:
                with tempfile.NamedTemporaryFile(
                    suffix=filename[filename.rfind(".") :], delete=False
                ) as f:
                    f.write(content)
                    file_path = pathlib.Path(f.name)
    ...
    def test_chunk_repair_functionality(self):
        ...
        try:
            ...
            if hashes:
                ...
                peer_chunks = {
                    chunk_hash: test_data[: len(test_data) // 2]
                }  # Partial data
