...
class TestAuditLoggingService:
    ...
    async def test_log_multiple_events(self, audit_service):
        ...
        assert audit_service.sequence_number == len(events) + 1  # +1 for service start event
    ...
    async def test_query_events_basic(self, audit_service):
        ...
        await audit_service.log_event(
            ...
            "user1", "peer1", "auth", "login", "success"
        )
        await audit_service.log_event(
            EventType.FILE_CREATED, Severity.LOW,
            "user1", "peer1", "file.txt", "create", "success"
        )
        await audit_service.log_event(
            EventType.LOGIN_FAILURE, Severity.HIGH,
            "user2", "peer2", "auth", "login", "failure"
        )
    ...
    async def test_query_events_filtered(self, audit_service):
        ...
        await audit_service.log_event(
            EventType.LOGIN_FAILURE, Severity.HIGH,
            "user2", "peer1", "auth", "login", "failure"
        )
        await audit_service.log_event(
            EventType.FILE_CREATED, Severity.LOW,
            "user1", "peer1", "file.txt", "create", "success"
        ...
        login_events = [e for e in events if e.event_type in [EventType.LOGIN_SUCCESS, EventType.LOGIN_FAILURE]]
    ...
    async def test_query_events_time_range(self, audit_service):
        ...
        await audit_service.log_event(
            ...
            "user1", "peer1", "file.txt", "delete", "success"
        )
        ...
        file_created_events = [e for e in events if e.event_type == EventType.FILE_CREATED]
        file_deleted_events = [e for e in events if e.event_type == EventType.FILE_DELETED]
    ...
    async def test_event_handlers(self, audit_service):
        ...
        await audit_service.log_event(
            EventType.LOGIN_SUCCESS, Severity.MEDIUM,
            "user2", "peer1", "auth", "login", "success"
        )
    ...
    async def test_security_violation_detection(self, audit_service):
        ...
        for i in range(6):  # Threshold is 5
            await audit_service.log_event(
                ...
                "suspicious_user", "peer1", "auth", "login", "failure"
            )
        ...
        violation_events = [e for e in events if e.event_type == EventType.SECURITY_VIOLATION]
