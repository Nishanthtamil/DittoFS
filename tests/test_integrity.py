...
class TestIntegrityVerifier:
    ...
    def test_get_repair_summary(self, verifier):
        ...
        result1 = RepairResult(status=RepairStatus.SUCCESS, chunk_hash="hash1")
        result2 = RepairResult(status=RepairStatus.FAILED, chunk_hash="hash2")
    ...
    def test_clear_old_reports(self, verifier):
        ...
        old_result = RepairResult(
            status=RepairStatus.SUCCESS, chunk_hash="old_hash", repair_time=old_time
        )
        new_result = RepairResult(
            status=RepairStatus.SUCCESS, chunk_hash="new_hash", repair_time=new_time
        )
...
class TestRecoveryProcedures:
    ...
    async def test_emergency_recovery(self, recovery, mock_verifier, mock_store):
        ...
        corruption_report = CorruptionReport(
            corruption_type=CorruptionType.CHUNK_MISSING, chunk_hash="hash1"
        )
        mock_verifier.verify_all_chunks.return_value = [corruption_report]

        # Mock repair results
        repair_result = RepairResult(status=RepairStatus.SUCCESS, chunk_hash="hash1")
...
class TestCorruptionReport:
    ...
    def test_init_with_defaults(self):
        ...
        report = CorruptionReport(
            corruption_type=CorruptionType.CHUNK_MISSING, chunk_hash="test_hash"
        )
...
class TestRepairResult:
    ...
    def test_init_with_defaults(self):
        ...
        result = RepairResult(status=RepairStatus.SUCCESS, chunk_hash="test_hash")
