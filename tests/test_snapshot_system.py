...
class TestSnapshotSystem:
    ...
    def test_create_incremental_snapshot(
        self, snapshot_system, crdt_store, demo_files, temp_dir
    ):
        ...
        full_snapshot = snapshot_system.create_snapshot(
            snapshot_type=SnapshotType.FULL, description="Full snapshot"
        )
    ...
    def test_snapshot_compression(self, snapshot_system, crdt_store, demo_files):
        ...
        # Create snapshot
        snapshot_id = snapshot_system.create_snapshot(
            snapshot_type=SnapshotType.FULL, description="Compressed snapshot"
        )
    ...
    def test_restore_full_snapshot(
        self, snapshot_system, crdt_store, demo_files, temp_dir
    ):
        ...
        # Create snapshot
        snapshot_id = snapshot_system.create_snapshot(
            snapshot_type=SnapshotType.FULL, description="Test snapshot"
        )
    ...
    def test_restore_selective_snapshot(
        self, snapshot_system, crdt_store, demo_files, temp_dir
    ):
    ...
    def test_snapshot_cleanup(self, snapshot_system):
        ...
        remaining = [
            s
            for s in snapshot_system.snapshot_cache.values()
    ...
    def test_differential_snapshot(
        self, snapshot_system, crdt_store, demo_files, temp_dir
    ):
        ...
        differential_snapshot = snapshot_system.create_snapshot(
            snapshot_type=SnapshotType.DIFFERENTIAL, description="Differential snapshot"
        )
    ...
    def test_restore_integrity_verification(
        self, snapshot_system, crdt_store, demo_files, temp_dir
    ):
        ...
        # Create snapshot
        snapshot_id = snapshot_system.create_snapshot(
            snapshot_type=SnapshotType.FULL, description="Integrity test snapshot"
        )
        ...
        restore_options = RestoreOptions(
            scope=RestoreScope.FULL, target_path=restore_dir, verify_integrity=True
        )
...
class TestSnapshotErrorHandling:
    ...
    def test_create_snapshot_without_crdt_store(self, temp_dir):
        ...
        snapshot_id = snapshot_system.create_snapshot(
            snapshot_type=SnapshotType.FULL, description="Test without CRDT"
        )
