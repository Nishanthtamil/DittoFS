...
-----END CERTIFICATE-----"""
            ...
            with tempfile.NamedTemporaryFile() as mock_validate:
                try:
                    ...
                    peer_identity = await alice_service.authenticate_peer(
                        "bob", mock_cert_pem
                    )
...
async def demo_challenge_response():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            result = await peer_service.verify_challenge_response(
                challenge2.challenge_id, b"wrong_response"
            )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            result = await peer_service.revoke_certificate(
                "malicious_peer", "Suspected malicious activity"
            )
            print(f"✓ Certificate revoked: {result}")

            # Check revocation status
            print(
                f"Revoked certificates count: {len(peer_service.revoked_certificates)}"
            )
            print(
                f"Certificate in revoked list: {mock_cert.fingerprint in peer_service.revoked_certificates}"
            )

            # Try to validate revoked certificate
            print(f"\nValidating revoked certificate...")
            status = await peer_service.validate_certificate(
                mock_cert.certificate_pem, "malicious_peer"
            )
...
async def demo_persistence():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create test data
        mock_cert = PeerCertificate(
            peer_id="bob", certificate_pem=b"mock_certificate_data"
        )
        ...
        try:
            ...
            if ("alice", "bob") in service2.trust_relationships:
                ...
                print(
                    f"✓ Trust relationship persisted: {relationship.trust_level.value}"
                )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if server_context:
                ...
                print(
                    f"  This is expected without cryptography libraries or certificates"
                )
