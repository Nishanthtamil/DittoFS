...
from src.dittofs.chunker import split, CHUNK_DIR
from src.dittofs.integrity import IntegrityVerifier, IntegrityScheduler, RecoveryProcedures
...
async def demo_integrity_verification():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        test_file2.write_text("This is test file 2 with different content for demonstration.")
        ...
        if reports:
            ...
            for report in reports:
                print(f"  - {report.corruption_type.value}: {report.chunk_hash[:16] if report.chunk_hash else 'N/A'}...")
        ...
        if hashes1:
            original_chunk_data = test_file1.read_bytes()[:len(original_chunk_data) if 'original_chunk_data' in locals() else 50]
            # Recreate the original chunk data
            test_file1_content = "This is test file 1 with some content for chunking."
            original_data[hashes1[0]] = test_file1_content.encode()[:50]  # Approximate chunk size
        
        # Store original data for missing chunk  
        if len(hashes2) > 0:
            test_file2_content = "This is test file 2 with different content for demonstration."
            original_data[hashes2[0]] = test_file2_content.encode()[:50]  # Approximate chunk size
        ...
        successful_repairs = sum(1 for r in repair_results if r.status.value == "success")
