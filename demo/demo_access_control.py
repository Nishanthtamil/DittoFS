    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            await access_service.update_role(
                "editor",
                description="Updated: Content editor with enhanced access"
            )
...
async def demo_permission_checking():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            users_to_test = ["admin_user", "regular_user", "readonly_user", "group_user", "unknown_user"]
            permissions_to_test = [Permission.READ, Permission.WRITE, Permission.DELETE, Permission.ADMIN]
            ...
            for username in users_to_test:
                ...
                print(f"    Effective permissions: {[p.value for p in user_perms.effective_permissions]}")
                
                for permission in permissions_to_test:
                    has_perm = await access_service.check_permission(username, permission)
            ...
            try:
                await access_service.require_permission("regular_user", Permission.ADMIN)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            for i, entry in enumerate(acl.entries):
                ...
                print(f"  {i+1}. {entry.access_type.value.upper()} {entry.principal_type} '{entry.principal}': {perms}")
            ...
            for username in test_users:
                has_read = await access_service.check_permission(username, Permission.READ, file_path)
                has_write = await access_service.check_permission(username, Permission.WRITE, file_path)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            for resource in test_resources:
                print(f"\n  Resource: {resource}")
                for username in test_users:
                    has_read = await access_service.check_permission(username, Permission.READ, resource)
                    has_write = await access_service.check_permission(username, Permission.WRITE, resource)
                    has_admin = await access_service.check_permission(username, Permission.ADMIN, resource)
                    
                    permissions = []
                    if has_read: permissions.append("READ")
                    if has_write: permissions.append("WRITE")
                    if has_admin: permissions.append("ADMIN")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            await access_service.create_role(
                ...
                {Permission.READ, Permission.WRITE, Permission.SHARE, Permission.MODIFY_PERMISSIONS}
            )
            ...
            for resource_path, owner, group in project_resources:
                ...
                print(f"✓ Created ACL for {resource_path} (owner: {owner}, group: {group})")
            ...
            test_scenarios = [
                ...
                ("bob", "/company/projects/project_a/src", "Senior dev access to source"),
                ("charlie", "/company/projects/project_a/src", "Junior dev access to source"),
                ("diana", "/company/confidential", "Readonly user denied confidential access"),
            ...
            for username, resource, description in test_scenarios:
                ...
                permissions_to_check = [Permission.READ, Permission.WRITE, Permission.DELETE, Permission.ADMIN]
                results = []
                
                for perm in permissions_to_check:
                    has_perm = await access_service.check_permission(username, perm, resource)
                    if has_perm:
...
async def demo_persistence():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            has_write_on_file = await service2.check_permission("user1", Permission.WRITE, "/test/file")
