...
from dittofs.chunker import split, join, CHUNK_DIR, ensure_chunk_dir
from dittofs.garbage_collector import (
    GarbageCollector, RetentionPolicy, ChunkReferenceTracker, GCStats
)
...
async def demo_reference_tracking():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            print(f"   Initial access time: {tracker.get_chunk_last_access(test_chunk)}")
            ...
            print(f"   Recently accessed: {tracker.is_chunk_recently_accessed(test_chunk, 3600)}")
            ...
            print(f"   All file2 chunks in missing: {file2_unique_chunks.issubset(chunks_from_missing)}")
...
async def demo_retention_policies():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            remaining = len([f for f in demo_chunk_dir.iterdir() 
                           if f.name.startswith('orphan_')])
            ...
            print(f"   Chunks deleted: {stats.chunks_deleted}")
            print(f"   Policy preserved chunks from missing files: {selective_policy.delete_from_missing_files == False}")
