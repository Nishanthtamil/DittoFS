...
from src.dittofs.versioning import MergeStrategy, RetentionPolicy
...
def demo_basic_versioning():
    ...
    try:
        ...
        version1_id = store.create_file_version(
            file_path, initial_content, commit_message="Initial document creation"
        )
        ...
        version3_id = store.create_file_version(
            file_path, final_content, commit_message="Final revisions and additions"
        )
...
def demo_version_comparison(store, file_path, version_ids):
    ...
    try:
        ...
        if comparison:
            print(
                f"✓ Versions are {'identical' if comparison['identical'] else 'different'}"
            )
            print(f"✓ Size difference: {comparison['size_diff']:+d} bytes")
            print(
                f"✓ Content type: {'Text' if comparison.get('is_text') else 'Binary'}"
            )
        ...
        for i in range(len(version_ids) - 1):
            ...
            if comp:
                print(
                    f"  {version_ids[i+1]} vs {version_ids[i]}: {comp['size_diff']:+d} bytes"
                )
...
def demo_branching_and_merging(store, file_path, version_ids):
    ...
    try:
        ...
        branch_id = store.create_branch(
            "feature-enhancement", base_version_id, "Branch for adding new features"
        )
        ...
        main_content = (
            b"Main branch updates\nParallel development\nMain branch features"
        )
        main_version_id = store.create_file_version(
            file_path, main_content, "main", "Parallel development on main branch"
        )
        ...
        feature_history = store.get_enhanced_version_history(
            file_path, "feature-enhancement"
        )
    ...
    try:
        ...
        for i in range(10):
            ...
            version_id = store.create_file_version(
                file_path, content, commit_message=f"Cleanup demo version {i}"
            )
