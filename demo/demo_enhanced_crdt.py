        ...
        if conflict:
            ...
            if result.get("success"):
                if "resolved_version" in result:
                    ...
                    print(
                        f"  Resolved to version with checksum: {resolved.get('checksum', 'N/A')}"
                    )
                    ...
                    for i, version in enumerate(result["preserved_versions"]):
                        print(
                            f"    Version {i+1}: {pathlib.Path(version['path']).name}"
                        )
...
def demo_version_history():
    ...
    for version in versions:
        print(
            f"  Version {version.version}: {version.checksum[:16]}... (size: {version.size})"
        )
    ...
    if conflict:
        ...
        if resolved_conflict_data:
            ...
            if resolved_conflict.resolution_strategy:
                print(
                    f"Resolution strategy: {resolved_conflict.resolution_strategy.value}"
                )
