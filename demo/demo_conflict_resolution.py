...
class ConflictResolutionDemo:
        ...
        if can_resolve:
            ...
            if success:
                ...
                if history:
                    ...
                    print(f"⏰ Resolved at: {entry.resolved_at.strftime('%Y-%m-%d %H:%M:%S')}")
            else:
        ...
        if diff:
            ...
            if diff.metadata_diff:
                ...
                for key, changes in diff.metadata_diff.items():
                    if isinstance(changes, dict) and 'local' in changes and 'remote' in changes:
    ...
    async def demo_batch_resolution(self):
        ...
        all_conflicts = [ConflictInfo.from_dict(data) for data in self.store.conflicts_map.values()]
