    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            for event in login_events:
                print(
                    f"  {event.timestamp.strftime('%H:%M:%S')} - {event.event_type.value} - {event.user_id} - {event.result}"
                )
            ...
            for event in high_severity_events:
                print(
                    f"  {event.timestamp.strftime('%H:%M:%S')} - {event.event_type.value} - {event.severity.value} - {event.user_id}"
                )
            ...
            for event in alice_events:
                print(
                    f"  {event.timestamp.strftime('%H:%M:%S')} - {event.event_type.value} - {event.action}"
                )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            suspicious_events = [
                ...
                # High activity user
                *[
                    (EventType.FILE_READ, Severity.LOW, "power_user", "success")
                    for _ in range(20)
                ],
            ]
            ...
            print(
                f"  Time range: {stats.get('time_range', {}).get('start', 'N/A')} to {stats.get('time_range', {}).get('end', 'N/A')}"
            )
            ...
            for user, count in sorted(
                stats.get("users", {}).items(), key=lambda x: x[1], reverse=True
            )[:5]:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            def security_handler(event):
                ...
                print(
                    f"🚨 SECURITY ALERT: {event.event_type.value} - {event.user_id} - {event.details}"
                )
            ...
            def file_handler(event):
                ...
                print(
                    f"📁 FILE OPERATION: {event.action} on {event.resource} by {event.user_id}"
                )

            # Register handlers
            audit_service.add_event_handler(
                EventType.SECURITY_VIOLATION, security_handler
            )
            audit_service.add_event_handler(
                EventType.LOGIN_FAILURE, login_failure_handler
            )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            for violation in violations:
                print(
                    f"  {violation.timestamp.strftime('%H:%M:%S')} - {violation.action} - {violation.user_id}"
                )
            ...
            if len(new_violations) > len(violations):
                print(
                    "✓ Additional security violation detected for privilege escalation"
                )
...
async def demo_retention_policy():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            print(
                f"Current log file size: {audit_service.current_log_file.stat().st_size} bytes"
            )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if audit_service.signing_key:
                ...
                print(
                    "⚠️  Cryptographic signing not available (requires cryptography library)"
                )
            ...
            print(f"  Current sequence number: {audit_service.sequence_number}")
            print(
                f"  Last hash: {audit_service.last_hash[:16]}..."
                if audit_service.last_hash
                else "  Last hash: None"
            )
...
async def demo_persistence():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            print(
                f"✓ Sequence number preserved: {service2.sequence_number >= original_sequence}"
            )
            ...
            for event in persistent_events:
                print(
                    f"  {event.timestamp.strftime('%H:%M:%S')} - {event.event_type.value} - {event.resource}"
                )
