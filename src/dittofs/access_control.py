...
class AccessControlList:
    ...
    def remove_entry(self, principal: str, principal_type: str) -> bool:
        ...
        self.entries = [
            ...
            if not (
                entry.principal == principal and entry.principal_type == principal_type
            )
        ]
    ...
    def get_entries_for_principal(
        self, principal: str, principal_type: str
...
class UserPermissions:
    ...
    def compute_effective_permissions(self) -> None:
        ...
        self.effective_permissions = (
            self.direct_permissions | self.role_permissions | self.group_permissions
        )
...
class AccessControlService:
    ...
    async def get_user_permissions(self, username: str) -> UserPermissions:
        ...
        if username in self.permission_cache:
            ...
            if (
                datetime.utcnow() - self.last_cache_clear
    ...
    async def check_permission(
        self, username: str, permission: Permission, resource_path: Optional[str] = None
    ) -> bool:
        """
        ...
        if user_perms.has_permission(permission):
            ...
            if resource_path:
                return await self._check_acl_permission(
                    username, permission, resource_path
                )
            return True
    ...
    async def remove_acl_entry(
        self, resource_path: str, principal: str, principal_type: str
    ) -> bool:
        """
    ...
    async def _check_acl_permission(
        self, username: str, permission: Permission, resource_path: str
    ) -> bool:
    ...
    def _save_data(self):
        ...
        try:
            ...
            # Save groups
            groups_data = {name: group.to_dict() for name, group in self.groups.items()}
            self.groups_file.write_text(json.dumps(groups_data, indent=2))

            # Save ACLs
            acls_data = {path: acl.to_dict() for path, acl in self.acls.items()}
