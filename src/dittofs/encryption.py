...
class EncryptedChunk:
    ...
    def to_bytes(self) -> bytes:
        ...
        return (self.salt + self.iv + self.hmac_tag + 
                key_id_len + key_id_bytes + self.encrypted_data)
...
class ChunkEncryption:
    ...
    def decrypt_chunk(self, encrypted_chunk: EncryptedChunk, chunk_hash: str) -> bytes:
        ...
        # Verify HMAC first
        h = hmac.HMAC(chunk_key, hashes.SHA256(), backend=default_backend())
        h.update(encrypted_chunk.salt + encrypted_chunk.iv + encrypted_chunk.encrypted_data)
    ...
    def verify_chunk_integrity(self, encrypted_chunk: EncryptedChunk, chunk_hash: str) -> bool:
...
def create_encryption_system(password: str, key_store_path: Optional[Path] = None) -> Tuple[KeyManager, ChunkEncryption]:
...
def load_encryption_system(password: str, key_store_path: Optional[Path] = None) -> Optional[Tuple[KeyManager, ChunkEncryption]]:
