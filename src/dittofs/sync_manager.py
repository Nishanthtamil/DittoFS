...
from .integrity import IntegrityScheduler, IntegrityVerifier
from .transport import Connection, TransportManager
...
class SyncManager:
    ...
    def __init__(self, store: CRDTStore, transport_manager: TransportManager):
        ...
        self.chunk_requests = {}
        self.peer_id = (
            f"dittofs-{hashlib.md5(str(time.time()).encode()).hexdigest()[:8]}"
        )
    ...
    async def _sync_loop(self):
        ...
        while self.is_running:
            try:
                ...
                completed = [
                    peer_id for peer_id, task in self.sync_tasks.items() if task.done()
                ]
        ...
        try:
            ...
            if "state_vector" in response and response["state_vector"]:
                try:
                    ...
                    if our_update:
                        await connection.send_message(
                            {"type": "sync_update", "update_data": our_update.hex()}
                        )
                except Exception as e:
    ...
    async def _request_missing_chunks(self):
        ...
        try:
            ...
            for chunk_hash in list(missing_chunks)[:5]:  # Limit concurrent requests
                ...
                for peer in peers:
                    try:
                        connection = await self.transport.connect_best_peer(
                            peer.peer_id
                        )
                        if connection:
                            await connection.send_message(
                        ...
                        logging.debug(
                            f"Failed to request chunk from {peer.peer_id}: {e}"
                        )
            ...
            expired = [
                chunk_hash
                for chunk_hash, request_time in self.chunk_requests.items()
    ...
    async def handle_chunk_request(self, connection: Connection, message: dict):
        ...
        try:
            ...
            if chunk_path.exists():
                ...
                await connection.send_message(
                    {"type": "chunk_not_found", "chunk_hash": chunk_hash}
                )
    ...
    async def handle_integrity_check_request(
        self, connection: Connection, message: dict
    ):
        ...
        try:
            ...
            for chunk_hash in chunk_hashes[:10]:  # Limit to 10 chunks per request
                is_valid, report = await self.integrity_verifier.verify_chunk(
                    chunk_hash
                )
            ...
            logging.info(
                f"Handled integrity check request from {requester_id} for {len(chunk_hashes)} chunks"
            )
            ...
            for peer in peers:
                try:
                    ...
                    if connection:
                        ...
                        if (
                            response
                            and response.get("type") == "chunk_response"
                            and response.get("chunk_hash") == chunk_hash
                        ):
    ...
    try:
        ...
        async def status_reporter():
            while True:
                ...
                logging.info(
                    f"Status - Peer: {status['peer_id'][:8]}..., "
