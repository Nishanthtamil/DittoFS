                ...
                if self._is_dittofs_device(device, advertisement_data):
                    ...
                    self.discovered_devices[peer_id] = PeerInfo(
                        ...
                        metadata={
                            ...
                            "rssi": (
                                advertisement_data.rssi if advertisement_data else -999
                            ),
                        },
                    )
                    found_count += 1
    ...
    async def _setup_notifications(self):
        ...
        try:
            await self.client.start_notify(
                self.rx_char.uuid, self._notification_handler
            )
        except Exception as e:
        ...
        try:
            ...
            for i in range(0, len(full_data), mtu):
                ...
                await self.client.write_gatt_char(
                    self.tx_char.uuid, chunk, response=False
                )
        ...
        try:
            ...
            nearby_devices = await loop.run_in_executor(
                ...
                lambda: bluetooth.discover_devices(
                    duration=int(timeout), lookup_names=True
                ),
            )
    ...
    async def connect_peer(self, peer: PeerInfo) -> Optional[Connection]:
        ...
        try:
            ...
            await loop.run_in_executor(
                None, lambda: sock.connect((peer.address, peer.port))
            )
...
class SimpleBLEConnection(Connection):
    ...
    async def send_message(self, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(
                None, lambda: self.socket.send(length_prefix + data)
            )
            return True
    ...
    async def receive_message(self, timeout: float = 30.0) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Read length prefix
            length_data = await loop.run_in_executor(None, lambda: self.socket.recv(4))
            ...
            # Read message data
            data = await loop.run_in_executor(None, lambda: self.socket.recv(length))
            ...
            if peers:
                ...
                if conn:
                    ...
                    success = await conn.send_message(
                        {"type": "hello", "message": "Hello from DittoFS BLE!"}
                    )
