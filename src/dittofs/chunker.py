...
class RollingHash:
    ...
    def update(self, byte_val: int) -> int:
        ...
        if self.window_full:
            ...
            self.hash_value = (
                self.hash_value - (old_byte * self.base_power) % self.mod
            ) % self.mod

        # Add new byte with position-dependent mixing for uniform data
        mixed_byte = byte_val ^ (
            self.position & 0xFF
        )  # XOR with position for better distribution
    ...
    for i, byte_val in enumerate(data):
        ...
        if chunk_size >= max_size:
            ...
            # Secondary condition: for uniform data, use position-based boundaries
                ...
                chunk_size >= config["avg_size"] and (hash_val & 0xFF) < 4
            ):  # ~1.5% chance
                is_boundary = True
...
def optimize_chunk_boundaries(
    data: bytes, boundaries: List[int], file_type: str
) -> List[int]:
...
def split(file_path: pathlib.Path) -> List[str]:
    ...
    try:
        ...
        logging.info(
            f"Split {file_path} ({file_type}) into {len(hashes)} variable-size chunks"
        )
        return hashes
...
def _split_large_file(
    file_path: pathlib.Path, config: Dict, file_type: str
) -> List[str]:
    ...
    with open(file_path, "rb") as f:
        ...
        while True:
            ...
            for boundary in boundaries[
                :-1
            ]:  # Skip last boundary as it might be incomplete
    ...
    if is_encryption_enabled():
        ...
        if encryption_system:
            ...
            try:
                ...
                decrypted_data = chunk_encryption.decrypt_chunk(
                ...
                logging.warning(
                    f"Failed to decrypt chunk {chunk_hash}, trying as unencrypted: {e}"
                )
        ...
        if is_encryption_enabled():
            ...
            if encryption_system:
                ...
                try:
                    chunk_bytes = chunk_path.read_bytes()
                    encrypted_chunk = EncryptedChunk.from_bytes(chunk_bytes)
                    return chunk_encryption.verify_chunk_integrity(
                        encrypted_chunk, chunk_hash
                    )
                except Exception:
...
def analyze_deduplication_ratio(file_paths: List[pathlib.Path]) -> Dict:
    ...
    dedup_ratio = (
        1.0 - (unique_chunks_size / total_original_size)
