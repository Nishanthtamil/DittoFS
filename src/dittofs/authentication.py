...
class AuthenticationService:
    ...
    def __init__(self, data_dir: Path, password_policy: Optional[PasswordPolicy] = None):
        """
    ...
    async def create_user(self, username: str, password: str, 
                         ...
                         permissions: List[str] = None) -> UserAccount:
        """
    ...
    async def authenticate_user(self, credentials: UserCredentials) -> AuthToken:
        ...
        try:
            ...
            if AuthMethod.PASSWORD in user.auth_methods and credentials.password:
                if self._verify_password(credentials.password, user.password_hash, user.salt):
                    auth_success = True
            ...
            if AuthMethod.CERTIFICATE in user.auth_methods and credentials.certificate_data:
            ...
            if AuthMethod.TOTP in user.auth_methods:
                if not credentials.totp_code or not self._verify_totp(credentials.totp_code, user):
    ...
    def _validate_password(self, password: str) -> None:
        ...
        if len(password) < policy.min_length:
            raise PasswordPolicyError(f"Password must be at least {policy.min_length} characters")
    ...
    async def _verify_certificate(self, certificate_data: bytes, user: UserAccount) -> bool:
        ...
        try:
            if self.users_file.exists():
                ...
                for username, user_data in data.items():
                    ...
                    user_data['auth_methods'] = [AuthMethod(method) for method in user_data['auth_methods']]
        ...
        try:
            if self.sessions_file.exists():
                ...
                for token_id, session_data in data.items():
                    # Convert datetime strings back to datetime objects
                    session_data['issued_at'] = datetime.fromisoformat(session_data['issued_at'])
    ...
    async def _periodic_cleanup(self) -> None:
        ...
        while True:
            try:
                ...
                expired_tokens = [
                    token_id for token_id, token in self.active_sessions.items()
                    if token.is_expired
                ]
