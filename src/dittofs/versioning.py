...
class VersioningSystem:
            ...
            if versions_file.exists():
                with open(versions_file, 'r') as f:
                    ...
                    for version_id, version_data in versions_data.items():
                        self.version_cache[version_id] = VersionNode.from_dict(version_data)
            ...
            if branches_file.exists():
                with open(branches_file, 'r') as f:
                    ...
                    for branch_id, branch_data in branches_data.items():
                        self.branch_cache[branch_id] = VersionBranch.from_dict(branch_data)
    ...
    def create_branch(self, branch_name: str, parent_version: str, description: str = "") -> str:
        ...
        try:
            ...
            logging.info(f"Created branch '{branch_name}' ({branch_id}) from version {parent_version}")
            return branch_id
    ...
    def create_version(self, file_path: str, content: bytes, branch_id: str = "main", 
                      parent_versions: Optional[List[str]] = None, 
                      commit_message: str = "", use_delta: bool = True) -> str:
        ...
        try:
            ...
            if use_delta and parent_versions:
                delta_stored = self._store_with_delta(version_node, content, parent_versions[0])
                if not delta_stored:
            ...
            logging.info(f"Created version {version_id} for {file_path} on branch {branch_id}")
            return version_id
    ...
    def _store_with_delta(self, version_node: VersionNode, content: bytes, parent_version_id: str) -> bool:
        ...
        try:
            ...
            if compression_ratio > 0.8:  # If delta is more than 80% of original, store full
                return False
            ...
            logging.debug(f"Stored delta for {version_node.version_id} with {compression_ratio:.2%} compression")
            return True
    ...
    def _calculate_delta(self, old_content: bytes, new_content: bytes) -> Optional[bytes]:
    ...
    def _apply_delta(self, base_content: bytes, delta_data: bytes, delta_type: DeltaType) -> Optional[bytes]:
    ...
    def get_version_history(self, file_path: str, branch_id: Optional[str] = None, 
                           limit: int = 50) -> List[VersionNode]:
    ...
    def merge_branches(self, target_branch: str, source_branch: str, 
                      strategy: MergeStrategy = MergeStrategy.THREE_WAY) -> Optional[str]:
        ...
        try:
            ...
            for file_path in common_files:
                ...
                common_ancestor = self._find_common_ancestor(target_version.version_id, source_version.version_id)
    ...
    def _get_latest_versions_in_branch(self, branch_id: str) -> Dict[str, VersionNode]:
        ...
        for version_node in self.version_cache.values():
            if version_node.branch_id == branch_id:
                ...
                if (file_path not in latest_versions or 
                    version_node.created_at > latest_versions[file_path].created_at):
                    latest_versions[file_path] = version_node
        
        return latest_versions
    
    def _find_common_ancestor(self, version1_id: str, version2_id: str) -> Optional[VersionNode]:
    ...
    def _three_way_merge(self, ancestor: VersionNode, version1: VersionNode, 
                        version2: VersionNode) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                merged_text, conflicts = self._merge_text_content(ancestor_text, text1, text2)
    ...
    def _merge_text_content(self, ancestor: str, version1: str, version2: str) -> Tuple[str, List[str]]:
    ...
    def _create_merge_commit(self, target_branch: str, source_branch: str, 
                           merge_results: Dict[str, VersionNode]) -> str:
        ...
        try:
            ...
            for file_path, merged_version in merge_results.items():
                # Get parent versions
                target_version = self._get_latest_version_in_branch(file_path, target_branch)
                source_version = self._get_latest_version_in_branch(file_path, source_branch)
    ...
    def _get_latest_version_in_branch(self, file_path: str, branch_id: str) -> Optional[VersionNode]:
        """Get latest version of a file in a specific branch"""
        latest_version = None
        
        for version_node in self.version_cache.values():
            if (version_node.file_path == file_path and 
                version_node.branch_id == branch_id):
                if (latest_version is None or 
                    version_node.created_at > latest_version.created_at):
    ...
    def apply_retention_policy(self, file_path: str, policy: Optional[RetentionPolicy] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if policy.max_versions and len(versions) > policy.max_versions:
                ...
                for version in versions_to_remove:
                    ...
                    if (policy.keep_major_versions and version.version_type in [VersionType.MERGE, VersionType.SNAPSHOT]):
                        continue
                ...
                for version in versions:
                    if version.created_at < cutoff_time:
                        ...
                        if self._remove_version(version.version_id):
                            ...
                            errors.append(f"Failed to remove old version {version.version_id}")
            ...
            if policy.compress_old_versions and policy.compress_after_days:
                ...
                for version in versions:
                    if (version.created_at < compress_cutoff and 
                        not version.delta_hash):  # Not already using delta storage
                        
                        if self._compress_version(version.version_id):
                            compressed_count += 1
                        else:
                            errors.append(f"Failed to compress version {version.version_id}")
    ...
    def _compress_version(self, version_id: str) -> bool:
        ...
        try:
            ...
            parent_versions = self.get_version_history(version_node.file_path, version_node.branch_id)
            parent_version = None
            
            for v in parent_versions:
                if (v.version_id != version_id and 
                    v.created_at < version_node.created_at and
                    not v.delta_hash):  # Use a non-delta version as parent
                    parent_version = v
                    break
            
            if parent_version:
                # Try to create delta
                if self._store_with_delta(version_node, content, parent_version.version_id):
