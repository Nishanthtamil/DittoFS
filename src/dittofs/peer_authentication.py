...
class PeerAuthenticationService:
    ...
    async def generate_peer_certificate(
        self, peer_id: str, validity_days: int = 365
    ) -> PeerCertificate:
        """
        ...
        if not CRYPTO_AVAILABLE:
            raise CertificateError(
                "Certificate generation requires cryptography libraries"
            )
        ...
        try:
            ...
            subject = issuer = x509.Name(
                [
                    ...
                    x509.NameAttribute(
                        NameOID.ORGANIZATIONAL_UNIT_NAME, "Peer Network"
                    ),
                ]
            )
    ...
    async def validate_certificate(
        self, certificate_pem: bytes, peer_id: str
    ) -> CertificateStatus:
        """
        ...
        try:
            ...
            if subject_cn != peer_id:
                logger.warning(
                    f"Certificate subject mismatch: expected {peer_id}, got {subject_cn}"
                )
                return CertificateStatus.INVALID
    ...
    async def authenticate_peer(
        self, peer_id: str, certificate_pem: bytes
    ) -> PeerIdentity:
        """
        ...
        if status != CertificateStatus.VALID:
            raise PeerAuthenticationError(
                f"Certificate validation failed: {status.value}"
            )
    ...
    async def verify_challenge_response(
        self, challenge_id: str, response: bytes
    ) -> bool:
        """
        ...
        try:
            ...
            if self.peers_file.exists():
                ...
                for peer_id, peer_data in data.items():
                    peer_data["last_seen"] = datetime.fromisoformat(
                        peer_data["last_seen"]
                    )
            ...
            if self.trust_file.exists():
                ...
                for key_str, trust_data in data.items():
                    ...
                    trust_data["established_at"] = datetime.fromisoformat(
                        trust_data["established_at"]
                    )
                    trust_data["last_interaction"] = datetime.fromisoformat(
                        trust_data["last_interaction"]
                    )
        ...
        try:
            ...
            for peer_id, cert in self.certificates.items():
                cert_data[peer_id] = {
                    ...
                    "private_key_pem": (
                        cert.private_key_pem.decode() if cert.private_key_pem else None
                    ),
                    "public_key_pem": (
                        cert.public_key_pem.decode() if cert.public_key_pem else ""
                    ),
            ...
            for peer_id, peer in self.peers.items():
                peer_data[peer_id] = {
                    ...
                    "certificate": {
                        ...
                        "public_key_pem": (
                            peer.certificate.public_key_pem.decode()
                            if peer.certificate.public_key_pem
                            else ""
                        ),
