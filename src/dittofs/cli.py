...
from .chunker import join, split
...
class DittoFSCLI:
    ...
    async def cmd_get(self, args):
        ...
        try:
            from .chunker import CHUNK_DIR, join
            ...
            if args.chunk:
                ...
                if not reports:
                    ...
                    for report in reports:
                        print(
                            f"  {report.corruption_type.value}: {report.error_message}"
                        )
                    return 1
                ...
                if not reports:
                    ...
                    if args.verbose:
                        ...
                        for report in reports[:10]:  # Show first 10
                            print(
                                f"  {report.chunk_hash[:16]}... - {report.error_message}"
                            )
                        if len(reports) > 10:
    ...
    async def cmd_repair(self, args):
        ...
        try:
            ...
            if self.sync_manager:
                verifier.register_peer_provider(
                    "sync_manager", self.sync_manager._get_chunk_from_peers
                )
            ...
            if args.chunk:
                ...
                if result.status.value == "success":
                    print(
                        f"✓ Chunk repaired successfully from peer: {result.peer_source}"
                    )
                    return 0
                else:
                ...
                try:
                    ...
                    if args.verbose and results:
                        ...
                        for result in results[:10]:
                            status_icon = (
                                "✓" if result.status.value == "success" else "✗"
                            )
                            print(
                                f"  {status_icon} {result.chunk_hash[:16]}... - {result.status.value}"
                            )
            ...
            if args.action == "run":
                ...
                if any([args.min_age, args.max_deletions, args.min_free_space]):
                    ...
                    if args.min_free_space:
                        policy.min_free_space_bytes = (
                            args.min_free_space * 1024 * 1024
                        )  # Convert MB to bytes
                ...
                print(
                    f"  Bytes freed: {stats.bytes_freed:,} bytes ({stats.bytes_freed / 1024 / 1024:.2f} MB)"
                )
                ...
                print("=" * 40)
                print(
                    f"Free space: {storage_info['free_bytes']:,} bytes ({storage_info['free_bytes'] / 1024 / 1024:.2f} MB)"
                )
                print(
                    f"Total space: {storage_info['total_bytes']:,} bytes ({storage_info['total_bytes'] / 1024 / 1024:.2f} MB)"
                )
                print(
                    f"Chunk storage: {storage_info['chunk_bytes']:,} bytes ({storage_info['chunk_bytes'] / 1024 / 1024:.2f} MB)"
                )
                ...
                avg_duration = sum(s.duration_seconds for s in stats_history) / len(
                    stats_history
                )

                print(f"Total chunks deleted: {total_deleted:,}")
                print(
                    f"Total bytes freed: {total_freed:,} bytes ({total_freed / 1024 / 1024:.2f} MB)"
                )
                ...
                if args.verbose:
                    ...
                    for stats in stats_history[-10:]:  # Show last 10 runs
                        ...
                        print(
                            f"{timestamp}: {stats.chunks_deleted} deleted, "
    ...
    async def cmd_snapshot(self, args):
        ...
        try:
            ...
            if args.action == "create":
                ...
                snapshot_type = (
                    SnapshotType(args.type) if args.type else SnapshotType.INCREMENTAL
                )
                ...
                if snapshot_id:
                    ...
                    if info:
                        ...
                        if metadata["compressed_size"] > 0:
                            ...
                            print(
                                f"  Compressed: {metadata['compressed_size']:,} bytes ({ratio:.1f}% saved)"
                            )
                    return 0
                else:
                ...
                for snapshot in snapshots:
                    created = time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(snapshot.created_at)
                    )
                ...
                success = snapshot_system.restore_snapshot(
                    args.snapshot_id, restore_options
                )
                ...
                created = time.strftime(
                    "%Y-%m-%d %H:%M:%S", time.localtime(metadata["created_at"])
                )
                ...
                if metadata["compressed_size"] > 0:
                    ...
                    print(
                        f"Compressed size: {metadata['compressed_size']:,} bytes ({ratio:.1f}% saved)"
                    )
                ...
                if args.verbose and files:
                    ...
                    for file_info in files[:20]:  # Show first 20 files
                        mtime = time.strftime(
                            "%Y-%m-%d %H:%M:%S", time.localtime(file_info["mtime"])
                        )
                        print(
                            f"  {file_info['path']} ({file_info['size']:,} bytes, {mtime})"
                        )
                ...
                if args.policy_action == "create":
                    ...
                    policy = SnapshotPolicy(
                        ...
                        snapshot_type=(
                            SnapshotType(args.type)
                            if args.type
                            else SnapshotType.INCREMENTAL
                        ),
    ...
    async def cmd_conflicts(self, args):
        ...
        try:
            ...
            if args.action == "list":
                # List conflicts
                conflicts = (
                    self.store.get_pending_conflicts() if args.pending_only else []
                )
                ...
                for conflict in conflicts:
                    ...
                    print(
                        f"{status_icon} {conflict.conflict_id[:16]}... - {conflict.file_path}"
                    )
                ...
                if conflict.resolved_at:
                    print(
                        f"Resolved: {conflict.resolved_at.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
                        ...
                        if diff.binary_file:
                            ...
                            if diff.metadata_diff:
                                ...
                                for key, changes in diff.metadata_diff.items():
                                    print(
                                        f"  {key}: {changes['local']} → {changes['remote']}"
                                    )
                    else:
                ...
                print(
                    f"Resolving conflict {args.conflict_id[:16]}... with {action.value}"
                )

                success = await workflow.manual_resolve_conflict(
                    args.conflict_id, action
                )
                ...
                if args.batch:
                    ...
                    if results["details"] and args.verbose:
                        ...
                        for detail in results["details"]:
                            status_icon = (
                                "✅" if detail["status"] == "auto_resolved" else "⚠️"
                            )
                            print(
                                f"  {status_icon} {detail['file_path']}: {detail['status']}"
                            )
                ...
                history = workflow.get_resolution_history(
                    file_path=args.file, limit=args.limit
                )
                ...
                for entry in history:
                    result_icon = (
                        "✅" if entry.result == ResolutionResult.SUCCESS else "❌"
                    )
                    print(
                        f"{result_icon} {entry.resolution_id[:16]}... - {entry.file_path}"
                    )
...
def build_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="dittofs", description="Distributed offline file system"
    )
    ...
    list_parser.add_argument(
        "--missing", action="store_true", help="Show missing chunks"
    )
    ...
    sync_parser.add_argument(
        "--timeout", type=float, default=10.0, help="Discovery timeout"
    )
    ...
    recover_group.add_argument(
        "--emergency", action="store_true", help="Full emergency recovery"
    )
    ...
    gc_parser.add_argument(
        "action",
        choices=["run", "status", "stats", "schedule"],
        help="GC action to perform",
    )

    # GC run options
    gc_parser.add_argument(
        "--force", action="store_true", help="Force GC even if already running"
    )
    gc_parser.add_argument(
        "--min-age",
        type=int,
    ...
    # GC schedule options
    gc_parser.add_argument("--start", action="store_true", help="Start GC scheduler")
    gc_parser.add_argument("--stop", action="store_true", help="Stop GC scheduler")
    gc_parser.add_argument(
        "--interval",
        type=int,
        metavar="SECONDS",
    ...
    # Restore options
    snapshot_parser.add_argument(
        "--snapshot-id", help="Snapshot ID for restore/delete/info"
    )
    snapshot_parser.add_argument("--target", help="Target directory for restore")
    snapshot_parser.add_argument(
        "--selective", action="store_true", help="Selective restore mode"
    )
    ...
    snapshot_parser.add_argument(
        "--no-permissions", action="store_true", help="Don't restore file permissions"
    )
    snapshot_parser.add_argument(
    ...
    snapshot_parser.add_argument(
        "--schedule", help="Cron schedule (default: '0 2 * * *')"
    )
    snapshot_parser.add_argument(
        "--disabled", action="store_true", help="Create policy as disabled"
    )
    snapshot_parser.add_argument(
        "--max-snapshots", type=int, help="Maximum snapshots to keep"
    )
    snapshot_parser.add_argument("--max-age-days", type=int, help="Maximum age in days")
    snapshot_parser.add_argument(
        "--full-interval", type=int, help="Full snapshot interval (default: 7)"
    )
    snapshot_parser.add_argument(
        "--no-compress", action="store_true", help="Disable snapshot compression"
    )

    # Conflict resolution command
    conflict_parser = subparsers.add_parser(
        "conflicts", help="Conflict resolution management"
    )
    ...
    conflict_parser.add_argument(
        "--pending-only", action="store_true", help="Show only pending conflicts"
    )
    ...
    conflict_parser.add_argument(
        "--diff", action="store_true", help="Show detailed diff for conflict"
    )
    ...
    conflict_parser.add_argument(
        "--batch", action="store_true", help="Resolve all auto-resolvable conflicts"
    )

    # History options
    conflict_parser.add_argument(
        "--limit", type=int, default=20, help="Limit number of history entries"
    )
