...
class AuditLoggingService:
    ...
    def __init__(
        self, data_dir: Path, retention_policy: Optional[RetentionPolicy] = None
    ):
        """
    ...
    def add_event_handler(
        self, event_type: EventType, handler: Callable[[AuditEvent], None]
    ):
        """
    ...
    async def _read_log_file(
        self, log_file: Path, query: AuditQuery
    ) -> List[AuditEvent]:
        ...
        try:
            ...
            for line in lines:
                ...
                try:
                    ...
                    if (
                        query.start_time
                        and log_entry.event.timestamp < query.start_time
                    ):
                        continue
    ...
    def _filter_events(
        ...
        if query.resources:
            filtered = [
                e for e in filtered if any(r in e.resource for r in query.resources)
            ]
    ...
    def _analyze_security_findings(self, events: List[AuditEvent]) -> List[str]:
        ...
        if critical_events:
            findings.append(
                f"Critical security events detected: {len(critical_events)}"
            )
        ...
        avg_activity = (
            sum(user_activity.values()) / len(user_activity) if user_activity else 0
        )
        for user, count in user_activity.items():
            if count > avg_activity * 3:  # 3x average activity
                findings.append(
                    f"Unusual activity level for user {user}: {count} events"
                )

        return findings

    def _generate_recommendations(
        self, events: List[AuditEvent], findings: List[str]
    ) -> List[str]:
        ...
        if any("access denied" in f for f in findings):
            ...
            recommendations.append(
                "Provide additional user training on proper access procedures"
            )
        ...
        recommendations.append(
            "Ensure audit log retention policies meet compliance requirements"
        )
    ...
    async def _check_security_violations(self, event: AuditEvent):
        ...
        if event.event_type == EventType.LOGIN_FAILURE:
            recent_failures = await self._count_recent_events(
                EventType.LOGIN_FAILURE, user_id=event.user_id, minutes=15
            )
        ...
        if (
            ...
            and event.severity == Severity.HIGH
        ):
            await self.log_event(
                EventType.SECURITY_VIOLATION,
                Severity.MEDIUM,
    ...
    def _get_archive_files_for_period(
