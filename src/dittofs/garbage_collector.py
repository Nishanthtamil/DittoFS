...
class ChunkReferenceTracker:
    ...
    def get_chunk_references(self) -> Dict[str, Set[str]]:
        ...
        with self._lock:
            ...
            logging.debug(f"Built reference cache with {len(self._reference_cache)} chunks")
            return self._reference_cache.copy()
    ...
    def get_orphaned_chunks(self) -> Set[str]:
        ...
        logging.debug(f"Found {len(orphaned)} orphaned chunks out of {len(disk_chunks)} total")
...
class GarbageCollector:
    ...
    async def collect_garbage(self, force: bool = False) -> GCStats:
        ...
        try:
            ...
            logging.info(f"Garbage collection completed: {stats.chunks_deleted} chunks deleted, "
    ...
    async def _find_deletion_candidates(self, emergency_mode: bool) -> List[str]:
        ...
        if emergency_mode:
            ...
            for chunk_hash in all_chunks:
                if not self.tracker.is_chunk_recently_accessed(
                    chunk_hash, self.policy.access_grace_period // 2  # Halve grace period
                ):
        ...
        logging.debug(f"Found {len(aged_candidates)} deletion candidates after age filtering")
        return aged_candidates
    
    async def _get_orphaned_chunks(self) -> Set[str]:
        """Get orphaned chunks (async wrapper)"""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(self._executor, self.tracker.get_orphaned_chunks)
    
    async def _get_chunks_from_missing_files(self) -> Set[str]:
        """Get chunks from missing files (async wrapper)"""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(self._executor, self.tracker.get_chunks_from_missing_files)
    ...
    async def _prioritize_candidates(self, candidates: List[str]) -> List[str]:
        ...
        return await loop.run_in_executor(
            self._executor, 
            lambda: sorted(candidates, key=_sort_key, reverse=True)
        )
            ...
            try:
                ...
                if chunk_path.exists():
                    ...
                    if await self._is_safe_to_delete(chunk_hash):
                        ...
                        logging.warning(f"Chunk {chunk_hash} not safe to delete, skipping")
    ...
    async def _is_safe_to_delete(self, chunk_hash: str) -> bool:
        ...
        if self.tracker.is_chunk_recently_accessed(chunk_hash, self.policy.access_grace_period):
            return False
...
def get_garbage_collector(store: Optional[CRDTStore] = None, 
                         policy: Optional[RetentionPolicy] = None) -> GarbageCollector:
