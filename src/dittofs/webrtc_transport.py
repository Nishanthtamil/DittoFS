...
from .transport_manager import Connection, PeerInfo, TransportProtocol
                    ...
                    async for message in websocket:
                        ...
                        if msg_type == "register":
                            ...
                            await websocket.send(
                                json.dumps({"type": "registered", "peer_id": peer_id})
                            )
...
class WebRTCTransport(TransportProtocol):
    ...
    async def start_server(self, port: int = None) -> bool:
        ...
        try:
            ...
            await self.signaling_ws.send(
                json.dumps({"type": "register", "peer_id": self.peer_id})
            )
        ...
        try:
            ...
            if source_peer in self.peer_connections and candidate_data:
                ...
                candidate = RTCIceCandidate(
                    candidate_data.get("candidate"), candidate_data.get("sdpMLineIndex")
                )
    ...
    async def discover_peers(self, timeout: float = 10.0) -> List[PeerInfo]:
        ...
        try:
            ...
            await self.signaling_ws.send(
                json.dumps({"type": "list-peers", "peer_id": self.peer_id})
            )
...
class StandaloneSignalingServer:
    ...
    async def _handle_message(self, websocket, data, current_peer_id, current_room):
        ...
        if msg_type == "join":
            ...
            if not peer_id:
                await websocket.send(
                    json.dumps({"type": "error", "message": "peer_id required"})
                )
                return
            ...
            await self._broadcast_to_room(
                room, {"type": "peer_joined", "peer_id": peer_id}, exclude=peer_id
            )
            ...
            if room in self.rooms:
                peers_list = [
                    pid for pid in self.rooms[room].keys() if pid != current_peer_id
                ]
                await websocket.send(
                    json.dumps({"type": "peer_list", "peers": peers_list})
                )
            ...
            if target_peer and target_peer in self.peers:
                ...
                await websocket.send(
                    json.dumps(
                        {"type": "error", "message": f"Peer {target_peer} not found"}
                    )
                )
            ...
            if not self.rooms[room]:
                ...
                asyncio.create_task(
                    self._broadcast_to_room(
                        room, {"type": "peer_left", "peer_id": peer_id}
                    )
                )
