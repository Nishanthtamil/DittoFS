...
from .versioning import VersioningSystem, RetentionPolicy
...
class ConflictInfo:
    ...
    def to_dict(self) -> dict:
        ...
        if isinstance(self.detected_at, datetime):
            ...
            data['detected_at'] = datetime.fromtimestamp(self.detected_at, tz=timezone.utc).isoformat()
        
        if self.resolved_at:
            if isinstance(self.resolved_at, datetime):
                data['resolved_at'] = self.resolved_at.isoformat()
            elif isinstance(self.resolved_at, (int, float)):
                data['resolved_at'] = datetime.fromtimestamp(self.resolved_at, tz=timezone.utc).isoformat()
    ...
    def from_dict(cls, data: dict) -> 'ConflictInfo':
        ...
        if isinstance(data['detected_at'], str):
            ...
            data['detected_at'] = datetime.fromtimestamp(data['detected_at'], tz=timezone.utc)
            
        if data.get('resolved_at'):
            if isinstance(data['resolved_at'], str):
                data['resolved_at'] = datetime.fromisoformat(data['resolved_at'])
            elif isinstance(data['resolved_at'], (int, float)):
                data['resolved_at'] = datetime.fromtimestamp(data['resolved_at'], tz=timezone.utc)
...
class CRDTStore:
    ...
    def _detect_conflict(self, new_record: FileRecord, existing_record: FileRecord) -> Optional[ConflictInfo]:
        """Detect conflicts between file versions"""
        conflicts = []
        
        # Check for timestamp conflicts
        if abs(new_record.mtime - existing_record.mtime) < 1.0 and new_record.checksum != existing_record.checksum:
        ...
        if (new_record.semantic_hash and existing_record.semantic_hash and 
            new_record.semantic_hash != existing_record.semantic_hash and
            new_record.checksum != existing_record.checksum):
        ...
        if conflicts:
            conflict_id = hashlib.sha256(f"{new_record.path}_{new_record.mtime}_{existing_record.mtime}".encode()).hexdigest()[:16]
            primary_conflict = conflicts[0]  # Use the first detected conflict as primary
    ...
    def _resolve_automatic_merge(self, conflict: ConflictInfo) -> Dict[str, Any]:
        ...
        versions = conflict.versions
        if len(versions) != 2:
            return {"success": False, "error": "Automatic merge requires exactly 2 versions"}
    ...
    def _resolve_semantic_merge(self, conflict: ConflictInfo) -> Dict[str, Any]:
        ...
        versions = conflict.versions
        if len(versions) != 2:
            return {"success": False, "error": "Semantic merge requires exactly 2 versions"}
        ...
        if file_path.suffix.lower() not in ['.txt', '.md', '.py', '.js', '.html', '.css', '.json']:
            return {"success": False, "error": "Semantic merge only supported for text files"}
        
        # For now, fall back to automatic merge
        return self._resolve_automatic_merge(conflict)
    
    def resolve_conflict(self, conflict_id: str, strategy: Optional[ConflictResolutionStrategy] = None, 
        ...
        try:
            ...
            if resolution_strategy == ConflictResolutionStrategy.MANUAL_RESOLUTION and manual_resolution:
            ...
            if resolution_result.get("success"):
                ...
                if "resolved_version" in resolution_result:
                    ...
                    resolved_record = FileRecord.from_dict(resolution_result["resolved_version"])
                ...
                logging.info(f"Resolved conflict {conflict_id} using {resolution_strategy.value}")
                return True
            else:
                # Resolution failed
                ...
                logging.warning(f"Failed to resolve conflict {conflict_id}: {resolution_result.get('error', 'Unknown error')}")
                return False
        ...
        try:
            for conflict_id, conflict_data in self.conflicts_map.items():
                try:
                    ...
                    logging.warning(f"Skipping corrupted conflict record {conflict_id}: {e}")
            return conflicts
        except Exception as e:
    ...
    def get_pending_conflicts(self) -> List[ConflictInfo]:
        ...
        return [c for c in self.get_conflicts() 
        ...
        try:
            ...
            if existing_record and existing_record.checksum != new_record.checksum:
                ...
                if conflict:
                    ...
                    logging.warning(f"Conflict detected for {path_str}: {conflict.conflict_type.value}")
                    ...
                    if self.default_strategy != ConflictResolutionStrategy.MANUAL_RESOLUTION:
                        self.resolve_conflict(conflict.conflict_id, self.default_strategy)
                    else:
                        logging.info(f"Conflict {conflict.conflict_id} awaiting manual resolution")
            ...
            logging.info(f"Added file: {new_record.path} with {len(hashes)} chunks (version {new_record.version})")
            return True
    ...
    def get_file_versions(self, file_path: pathlib.Path, limit: int = 10) -> List[FileRecord]:
            ...
            for version_key, version_data in self.versions_map.items():
                if version_key.startswith(f"{path_str}_"):
                    try:
                        ...
                        logging.warning(f"Skipping corrupted version record {version_key}: {e}")
    ...
    def create_file_version(self, file_path: pathlib.Path, content: bytes, 
                           branch_name: str = "main", commit_message: str = "") -> Optional[str]:
    ...
    def get_enhanced_version_history(self, file_path: pathlib.Path, 
                                   branch_name: Optional[str] = None, 
                                   limit: int = 50) -> List[Dict[str, Any]]:
        """Get enhanced version history with full version tree information"""
        try:
            path_str = str(file_path.resolve())
            version_nodes = self.versioning.get_version_history(path_str, branch_name, limit)
            ...
            for node in version_nodes:
                ...
                if enhanced_history:
                    ...
                    comparison = self.versioning.compare_versions(prev_version_id, node.version_id)
                    if comparison:
    ...
    def create_branch(self, branch_name: str, parent_version_id: str, description: str = "") -> Optional[str]:
        """Create a new branch from a parent version"""
        return self.versioning.create_branch(branch_name, parent_version_id, description)
    
    def merge_branches(self, target_branch: str, source_branch: str) -> Optional[str]:
        """Merge one branch into another"""
        from .versioning import MergeStrategy
        return self.versioning.merge_branches(target_branch, source_branch, MergeStrategy.THREE_WAY)
    
    def get_branches(self, file_path: Optional[pathlib.Path] = None) -> List[Dict[str, Any]]:
    ...
    def compare_file_versions(self, version1_id: str, version2_id: str) -> Optional[Dict[str, Any]]:
    ...
    def cleanup_old_versions(self, file_path: Optional[pathlib.Path] = None, 
                           policy: Optional[RetentionPolicy] = None) -> Dict[str, Any]:
    ...
    def merge_updates(self, update_data: bytes) -> bool:
        ...
        try:
                ...
                if path in current_files:
                    ...
                    if isinstance(new_data, dict) and isinstance(current_data, dict):
                        try:
                            ...
                            if (new_record.checksum != current_record.checksum or 
                                new_record.mtime != current_record.mtime):
                                conflict = self._detect_conflict(new_record, current_record)
                                if conflict:
                                    new_conflicts.append(conflict)
                        except Exception as e:
                            logging.warning(f"Error checking for conflicts in {path}: {e}")
            ...
            for conflict in new_conflicts:
                ...
                logging.warning(f"Merge conflict detected for {conflict.file_path}: {conflict.conflict_type.value}")
            ...
            if new_conflicts:
                logging.info(f"Merged CRDT update with {len(new_conflicts)} conflicts detected")
            else:
