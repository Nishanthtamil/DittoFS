...
from .chunker import CHUNK_DIR, _retrieve_chunk, _store_chunk, verify_chunk_integrity
...
class IntegrityVerifier:
    ...
    async def verify_chunk(
        self, chunk_hash: str
    ) -> Tuple[bool, Optional[CorruptionReport]]:
        ...
        try:
            ...
            if self.chunk_dir.exists():
                for chunk_file in self.chunk_dir.iterdir():
                    if (
                        chunk_file.is_file() and len(chunk_file.name) == 64
                    ):  # BLAKE3 hash length
                        all_chunk_hashes.add(chunk_file.name)

            self.integrity_logger.info(
                f"Starting integrity check of {len(all_chunk_hashes)} chunks"
            )
            ...
            for chunk_hash in all_chunk_hashes:
                ...
                if not is_valid and report:
                    ...
                    self.integrity_logger.warning(
                        f"Corruption detected: {report.to_dict()}"
                    )
    ...
    async def repair_chunk(
        self, chunk_hash: str, peer_sources: Optional[List[str]] = None
    ) -> RepairResult:
        ...
        try:
            ...
            for peer_id in sources_to_try:
                if peer_id in self.peer_chunk_providers:
                    try:
                        ...
                        self.integrity_logger.debug(
                            f"Failed to get chunk from peer {peer_id}: {e}"
                        )
                        continue
            ...
            if is_valid:
                ...
                self.integrity_logger.info(
                    f"Successfully repaired chunk {chunk_hash} from peer {peer_source}"
                )
                return result
            else:
                result = RepairResult(
    ...
    async def repair_all_corruptions(self) -> List[RepairResult]:
        ...
        self.integrity_logger.info(
            f"Attempting to repair {len(chunks_to_repair)} corrupted chunks"
        )
    ...
    def get_corruption_summary(self) -> Dict:
        ...
        for report in self.corruption_reports:
            ...
            summary["by_type"][corruption_type] = (
                summary["by_type"].get(corruption_type, 0) + 1
            )
    ...
    def clear_old_reports(self, max_age_hours: int = 168):  # 1 week default
        ...
        self.repair_results = [
            result for result in self.repair_results if result.repair_time > cutoff_time
        ]
...
class IntegrityScheduler:
    ...
    async def _scheduler_loop(self):
        ...
        try:
            while self.is_running:
                ...
                if (
                    current_time - self.verifier.stats.last_full_check
                ) > self.full_check_interval:
                    logging.info("Starting scheduled full integrity check")
                    await self._run_full_check()

                # Check if we need an incremental check
                elif (
                    current_time - self.verifier.stats.last_incremental_check
                ) > self.incremental_check_interval:
    ...
    async def _run_full_check(self):
        ...
        try:
            ...
            if reports:
                logging.warning(
                    f"Full integrity check found {len(reports)} corruptions"
                )
    ...
    async def _run_incremental_check(self):
        ...
        try:
            ...
            if reports:
                logging.info(
                    f"Incremental integrity check found {len(reports)} corruptions"
                )
            else:
                logging.debug(
                    "Incremental integrity check completed - no corruptions found"
                )
    ...
    async def _run_auto_repair(self):
        ...
        try:
            ...
            recent_repair_hashes = {
                result.chunk_hash
                for result in self.verifier.repair_results
                if result.repair_time > recent_threshold
                and result.status == RepairStatus.SUCCESS
            }
            ...
            for report in self.verifier.corruption_reports:
                if (
                    report.chunk_hash
                    and report.chunk_hash not in recent_repair_hashes
                    and report.corruption_type
                    in [
                        CorruptionType.CHUNK_MISSING,
                        CorruptionType.CHUNK_CORRUPTED,
                        CorruptionType.HASH_MISMATCH,
            ...
            if successful > 0:
                logging.info(
                    f"Auto-repair completed: {successful}/{len(results)} chunks repaired successfully"
                )
    ...
    def configure(
        ...
        logging.info(
            f"Integrity scheduler configured: "
    ...
    async def emergency_recovery(self) -> Dict:
        ...
        try:
            ...
            for file_record in files:
                try:
                    file_reports = await self.verifier.verify_file_integrity(
                        file_record
                    )
                    ...
                    if file_reports:
                        ...
                        file_chunks_repaired = any(
                            ...
                            for result in repair_results
                            if result.status == RepairStatus.SUCCESS
                            for chunk_hash in [result.chunk_hash]
                            if chunk_hash
                        )
