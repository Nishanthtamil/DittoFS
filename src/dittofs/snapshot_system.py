...
class SnapshotSystem:
    ...
    def __init__(self, storage_path: pathlib.Path, crdt_store=None, peer_id: str = "default"):
            ...
            if metadata_file.exists():
                with open(metadata_file, 'r') as f:
                    ...
                    for snapshot_id, snapshot_data in metadata_data.items():
                        self.snapshot_cache[snapshot_id] = SnapshotMetadata.from_dict(snapshot_data)
            ...
            if policies_file.exists():
                with open(policies_file, 'r') as f:
                    ...
                    for policy_name, policy_data in policies_data.items():
                        self.policy_cache[policy_name] = SnapshotPolicy.from_dict(policy_data)
        except Exception as e:
    ...
    def create_snapshot(self, snapshot_type: SnapshotType = SnapshotType.INCREMENTAL,
                       description: str = "", tags: List[str] = None,
                       parent_snapshot: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            files_to_snapshot = self._collect_files_for_snapshot(snapshot_type, parent_snapshot)
            ...
            for file_path, file_record in files_to_snapshot.items():
                try:
                    ...
                    logging.warning(f"Failed to include file {file_path} in snapshot: {e}")
            
            # Save snapshot data
            snapshot_data_file = snapshot_dir / "files.json"
            with open(snapshot_data_file, 'w') as f:
                json.dump({path: file.to_dict() for path, file in snapshot_files.items()}, f, indent=2)
            ...
            logging.info(f"Created snapshot {snapshot_id} with {len(snapshot_files)} files "
    ...
    def _collect_files_for_snapshot(self, snapshot_type: SnapshotType, 
                                   parent_snapshot: Optional[str] = None) -> Dict[str, Any]:
        ...
        if snapshot_type == SnapshotType.FULL:
            ...
            return self._get_changed_files_since_snapshot(current_files, parent_snapshot)
    ...
    def _find_latest_full_snapshot(self) -> Optional[str]:
        ...
        for snapshot_id, metadata in self.snapshot_cache.items():
            if (metadata.status == SnapshotStatus.COMPLETED and 
                metadata.snapshot_type == SnapshotType.FULL and
                metadata.created_at > latest_time):
                latest_time = metadata.created_at
                latest_snapshot = snapshot_id
    ...
    def _get_changed_files_since_snapshot(self, current_files: Dict[str, Any], 
                                         parent_snapshot_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_path, file_record in current_files.items():
                if file_path not in parent_snapshot_files:
                    ...
                    if (file_record.checksum != parent_file.checksum or
                        file_record.mtime > parent_file.mtime):
                        changed_files[file_path] = file_record
            
            logging.info(f"Found {len(changed_files)} changed files since snapshot {parent_snapshot_id}")
    ...
    def restore_snapshot(self, snapshot_id: str, options: RestoreOptions) -> bool:
        ...
        try:
            ...
            logging.info(f"Restoring snapshot {snapshot_id} with scope {options.scope.value}")
            ...
            logging.info(f"Restore completed: {restored_count} files restored, {failed_count} failed")
    ...
    def _load_snapshot_data(self, snapshot_id: str) -> Optional[Dict[str, SnapshotFileRecord]]:
    ...
    def _filter_files_for_restore(self, snapshot_files: Dict[str, SnapshotFileRecord],
                                 options: RestoreOptions) -> Dict[str, SnapshotFileRecord]:
        """Filter files based on restore options"""
        ...
        for file_path, snapshot_file in snapshot_files.items():
            ...
            if options.file_patterns:
                import fnmatch
                matches_pattern = any(fnmatch.fnmatch(file_path, pattern) 
            ...
            if options.exclude_patterns:
                import fnmatch
                matches_exclude = any(fnmatch.fnmatch(file_path, pattern) 
    ...
    def _restore_single_file(self, snapshot_file: SnapshotFileRecord, 
                           options: RestoreOptions) -> bool:
        """Restore a single file from snapshot"""
        try:
            # Determine target path
            if options.target_path:
                target_path = options.target_path / pathlib.Path(snapshot_file.file_path).name
            else:
                target_path = pathlib.Path(snapshot_file.file_path)
            
            # Check if file exists and handle overwrite
            if target_path.exists() and not options.overwrite_existing:
                logging.warning(f"File {target_path} exists, skipping (overwrite disabled)")
                return True
            ...
            if not join(snapshot_file.chunk_hashes, target_path):
                logging.error(f"Failed to restore file content for {snapshot_file.file_path}")
                return False
            ...
            if options.preserve_permissions:
                try:
                    ...
                    logging.warning(f"Failed to restore permissions for {target_path}: {e}")
            ...
            logging.debug(f"Successfully restored {snapshot_file.file_path} to {target_path}")
            return True
    ...
    def _verify_restored_file(self, file_path: pathlib.Path, 
                            snapshot_file: SnapshotFileRecord) -> bool:
    ...
    def cleanup_snapshots(self, policy_name: Optional[str] = None) -> int:
        ...
        try:
            ...
            for policy in policies_to_apply:
                ...
                for snapshot_id, metadata in self.snapshot_cache.items():
                    if (metadata.status == SnapshotStatus.COMPLETED and
                        (not policy.tags or any(tag in metadata.tags for tag in policy.tags))):
                ...
                if policy.max_snapshots and len(matching_snapshots) > policy.max_snapshots:
    ...
    def _schedule_policy(self, policy: SnapshotPolicy):
        ...
        def create_scheduled_snapshot():
            try:
                ...
                if (snapshot_type == SnapshotType.INCREMENTAL and 
                    policy.full_snapshot_interval > 0):
                    
                    # Count incremental snapshots since last full
                    incremental_count = 0
                    for metadata in self.snapshot_cache.values():
                        if (metadata.status == SnapshotStatus.COMPLETED and
                            any(tag in metadata.tags for tag in policy.tags) and
                            metadata.snapshot_type == SnapshotType.INCREMENTAL):
                            incremental_count += 1
                ...
                if snapshot_id:
                    logging.info(f"Created scheduled snapshot {snapshot_id} from policy '{policy.name}'")
                    
                    # Run cleanup after creating snapshot
                    self.cleanup_snapshots(policy.name)
                else:
                    logging.error(f"Failed to create scheduled snapshot from policy '{policy.name}'")
        ...
        if len(cron_parts) == 5:
            ...
            if day == "*" and month == "*" and weekday == "*":
                if hour.isdigit() and minute.isdigit():
                    schedule.every().day.at(f"{hour}:{minute}").do(create_scheduled_snapshot)
                    logging.info(f"Scheduled policy '{policy.name}' for daily at {hour}:{minute}")
...
def get_snapshot_system(storage_path: pathlib.Path = None, crdt_store=None, 
                       peer_id: str = "default") -> SnapshotSystem:
...
def initialize_snapshot_system(storage_path: pathlib.Path, crdt_store=None, 
                              peer_id: str = "default") -> SnapshotSystem:
