...
from .crdt_store import (
    ConflictInfo, ConflictState, ConflictType, ConflictResolutionStrategy,
    FileRecord, CRDTStore
)
...
class ConflictResolutionWorkflow:
    ...
    def __init__(self, crdt_store: CRDTStore, peer_id: str = None):
        ...
        self.resolution_history_path = crdt_store.store_path.parent / "resolution_history.json"
        ...
        self.auto_resolution_rules: List[Callable[[ConflictInfo], Optional[ResolutionAction]]] = [
    ...
    def _load_resolution_history(self):
        ...
        try:
            if self.resolution_history_path.exists():
                ...
                logging.debug(f"Loaded {len(self.resolution_history)} resolution history entries")
    ...
    def _add_resolution_entry(self, conflict_id: str, file_path: str, 
                            action: ResolutionAction, result: ResolutionResult,
                            strategy: ConflictResolutionStrategy, details: Dict[str, Any] = None):
        ...
        logging.info(f"Added resolution entry: {resolution_id} for conflict {conflict_id}")
    ...
    def can_auto_resolve(self, conflict: ConflictInfo) -> Tuple[bool, Optional[ResolutionAction]]:
        ...
        try:
            ...
            if success:
                self._add_resolution_entry(
                    conflict_id, conflict.file_path, action, 
                    ResolutionResult.SUCCESS, ConflictResolutionStrategy.AUTOMATIC_MERGE,
                    {"auto_resolved": True, "rule_applied": action.value}
                )
                logging.info(f"Auto-resolved conflict {conflict_id} using {action.value}")
                return True
            else:
                self._add_resolution_entry(
                    conflict_id, conflict.file_path, action,
                    ResolutionResult.FAILED, ConflictResolutionStrategy.AUTOMATIC_MERGE,
    ...
    async def _execute_resolution_action(self, conflict: ConflictInfo, 
                                       action: ResolutionAction) -> bool:
        ...
        try:
            ...
            if action == ResolutionAction.KEEP_LOCAL:
                ...
                resolved_version = self._merge_versions_automatically(versions[0], versions[1])
    ...
    def _merge_versions_automatically(self, version1: dict, version2: dict) -> Optional[dict]:
        ...
        try:
            ...
            merged['version'] = max(
                version1.get('version', 1),
                version2.get('version', 1)
            ) + 1
    ...
    async def _preserve_both_versions(self, conflict: ConflictInfo, versions: List[dict]) -> bool:
    ...
    def generate_conflict_diff(self, conflict_id: str) -> Optional[ConflictDiff]:
        ...
        try:
            ...
            if self._is_text_file(file_path):
                diff.content_diff = self._generate_text_diff(file_path, local_record, remote_record)
                diff.binary_file = False
            else:
                diff.binary_file = True
            
            # Generate metadata diff
            diff.metadata_diff = self._generate_metadata_diff(local_record, remote_record)
    ...
    def _is_text_file(self, file_path: pathlib.Path) -> bool:
        ...
        text_extensions = {
            ...
            '.go', '.rs', '.php', '.rb', '.pl', '.r', '.m', '.swift'
        }
        
        return file_path.suffix.lower() in text_extensions
    
    def _generate_text_diff(self, file_path: pathlib.Path, 
                          local_record: FileRecord, remote_record: FileRecord) -> str:
        ...
        try:
            ...
            remote_info = f"Remote version (modified: {time.ctime(remote_record.mtime)})"
    ...
    def _generate_metadata_diff(self, local_record: FileRecord, 
                              remote_record: FileRecord) -> Dict[str, Any]:
    ...
    async def manual_resolve_conflict(self, conflict_id: str, 
        ...
        try:
            ...
            if resolution_choice == ResolutionAction.CUSTOM_RESOLUTION and custom_data:
                ...
                success = await self._execute_resolution_action(conflict, resolution_choice)
            ...
            self._add_resolution_entry(
                conflict_id, conflict.file_path, resolution_choice,
                result, ConflictResolutionStrategy.MANUAL_RESOLUTION, details
            )
    ...
    async def _execute_custom_resolution(self, conflict: ConflictInfo, 
                                       custom_data: Dict[str, Any]) -> bool:
    ...
    def get_resolution_history(self, file_path: Optional[str] = None, 
                             limit: int = 100) -> List[ResolutionEntry]:
    ...
    def get_resolution_stats(self) -> Dict[str, Any]:
        ...
        successful = sum(1 for entry in self.resolution_history 
                        if entry.result == ResolutionResult.SUCCESS)
        
        auto_resolved = sum(1 for entry in self.resolution_history
        ...
        return {
            ...
            'recent_resolutions': len([
                entry for entry in self.resolution_history
